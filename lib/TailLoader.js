define(['jquery'], function ($) {
   ...
   *        }
   *      });
   *      // 绑定页尾事件(如通过$.tailLoader(options)生成尾加载实例，则会自动执行绑定)
   *      tailLoader.bindBottom();
   */
  var TailLoader = function (options) {
    var self = this;
  ...
  $.extend({
    ...
    tailLoader: function (options, bindBottom) {
      var loader;
      bindBottom = typeof bindBottom === 'undefined' ? true : !!bindBottom;
      loader = new TailLoader(options);
      if (bindBottom) {
        loader.bindBottom();
      }
      return loader;
